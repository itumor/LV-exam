#!/usr/bin/env python3
"""Regenerate exam attachment PNGs from Markdown descriptions using Gemini."""
...
def generate_image(*, api_key: str, prompt: str) -> tuple[str, bytes]:
    payload = json.dumps(
        {
    ...
    for candidate in data.get("candidates", []):
        ...
        for part in content.get("parts", []):
            ...
            if inline and inline.get("data"):
                ...
                return mime_type, inline["data"].encode("ascii")

    raise RuntimeError(f"Gemini response did not contain image data: {data}")


def write_image(target: Path, *, mime_type: str, image_b64: bytes) -> None:
    suffix = MIME_SUFFIX.get(mime_type, ".bin")
    target.parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="gemini-image-") as tmp_dir_raw:
        tmp_dir = Path(tmp_dir_raw)
        source_path = tmp_dir / f"source{suffix}"
        source_path.write_bytes(base64.b64decode(image_b64))

        tmp_png = tmp_dir / "normalized.png"
        subprocess.run(
            [
                "sips",
                "-s",
        ...
        tmp_target = target.with_suffix(".tmp.png")
        tmp_target.write_bytes(tmp_png.read_bytes())
...
def main() -> int:
    ...
    parser.add_argument("--limit", type=int, default=0, help="Only process the first N mapped images.")
    ...
    for image_path, description, exam_name in all_items:
        ...
        try:
            mime_type, image_b64 = generate_image(api_key=api_key, prompt=prompt)
        except urllib.error.HTTPError as exc:
            body = exc.read().decode("utf-8", errors="replace")
            if exc.code == 429:
                print(
                    "Gemini quota/rate limit blocked image generation. "
                    f"Target: {image_path}. Response: {body}",
                    file=sys.stderr,
                )
                return 1
            print(f"Gemini request failed for {image_path}: HTTP {exc.code} {body}", file=sys.stderr)
            return 1
        except urllib.error.URLError as exc:
            print(f"Failed to call Gemini for {image_path}: {exc}", file=sys.stderr)
            return 1
        except Exception as exc:
            print(f"Failed to generate {image_path}: {exc}", file=sys.stderr)
            return 1
        write_image(image_path, mime_type=mime_type, image_b64=image_b64)

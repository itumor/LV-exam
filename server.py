...
class EvaluationError(RuntimeError):
    def __init__(self, message: str, *, status_code: int = 502, retry_state: str = "failed") -> None:
        super().__init__(message)
        self.status_code = status_code
        self.retry_state = retry_state


class QuotaExceededError(EvaluationError):
    def __init__(self, message: str = "AI scoring quota exceeded.") -> None:
...
def evaluate_submission(submission: dict[str, Any], exam_markdown: str) -> dict[str, Any]:
    ...
    quota = enforce_scoring_quota(submission)
    last_error: Exception | None = None
    for attempt in range(3):
        try:
            result_payload = normalize_evaluation_payload(provider_call_once(config, submission_context, exam_context))
            break
        except TimeoutError as error:
            last_error = error
            if attempt == 2:
                raise EvaluationError("LLM scoring timed out.", status_code=504, retry_state="exhausted") from error
            time.sleep(2 ** attempt)
        except EvaluationError as error:
            last_error = error
            if attempt == 2:
                raise EvaluationError(str(error), status_code=error.status_code, retry_state="exhausted") from error
            time.sleep(2 ** attempt)
    else:
        raise EvaluationError(str(last_error or "LLM scoring failed."), status_code=502, retry_state="exhausted")

    result_payload["telemetry"] = {
        "identity": {"user_key": quota["user_key"]},
        "plan": quota["plan"],
        "quota": {"usage": quota["usage"], "limit": quota["limit"]},
    }
    AUDIT_LOG.append(
        {
            "submission_id": submission.get("submission_id"),
            ...
            "created_at": now_iso(),
            "score_total": result_payload.get("evaluation", {}).get("scores", {}).get("total"),
        }
    )
